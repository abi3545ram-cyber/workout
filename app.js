const { useState, useEffect, useRef, useMemo, useCallback } = React;
    ...
    const TENDON = {
      explosive:{label:"Power Plan",weeks:"~12 weeks",name:"Explosive + Tendon",color:"#ff453a",
        meta:"Two standalone tendon days, run non-consecutively (e.g. Mon + Fri, Friday before a rest day). These are separate sessions from your full-body lifting — not bolted onto a lift. Explosive work first while the CNS is fresh, ordered fastest → slowest, then the HSR + isometric loading that builds the tendon stiffness underneath it. Cap every explosive exercise by quality — the set ends the moment jump distance or ground-contact speed drops, never on rep count. Progress plyos/sprints by intensity, HSR/isos by load. Ramp the explosive volume in over 2-3 weeks and deload periodically.",
        sessions:[
          {label:"Warm-Up",day:"Shared — do before both days",exercises:[
            {id:"wu-p1",name:"Phase 1 — Raise temperature",equip:"Warm-up",sets:1,reps:"~3 min",weight:"",rest:"0s",unilateral:false,cue:"Easy jog (bike instead before Day A if the knees are cranky). Light sweat, slightly raised heart rate. Don't go into drills cold."},
            ...
            {id:"wu-p4",name:"Phase 4 — Day-specific ramp",equip:"Warm-up",sets:1,reps:"~2-3 min",weight:"",rest:"0s",unilateral:false,cue:"Day A (vertical): ankle hops 2×15, then 3-5 low pogos + 2-3 submax box step-off landings before depth jumps. Day B (sprint): 2-3 build-up runs (30m, 60→80→95%) + 1-2 near-max efforts before the working accelerations. No long static stretches before training."},
          ]},
          {label:"Day A",day:"Vertical / Reactive + HSR",exercises:[
            {id:"dA-warmup",name:"Warm-up (see Warm-Up tab)",equip:"Warm-up",sets:1,reps:"8-10 min",weight:"",rest:"0s",unilateral:false,cue:"Run the shared Warm-Up first. Day A ramp: ankle hops 2×15, then 3-5 low pogos + 2-3 submax box step-off landings before depth jumps."},
            {id:"dA-depth",name:"Depth Jumps (~30cm box)",equip:"Box ~30cm",sets:4,reps:"3",weight:"Bodyweight",rest:"2 min",cue:"Step off (don't jump), land, instantly rebound up. Minimal ground contact — fast/short SSC. Progress box height, never reps. Set ends the moment contact time lengthens."},
            {id:"dA-boxjump",name:"Box Jumps / CMJ",equip:"Box",sets:4,reps:"5",weight:"Bodyweight",rest:"90s",cue:"Concentric vertical power — the second, distinct jump quality. Full hip extension, land soft and quiet, max intent every rep. Stop the set if height drops."},
            {id:"dA-calf-hsr",name:"Heavy-Slow Calf Raise — Standing (gastroc)",equip:"Machine/DB",sets:4,reps:"6-8",tempo:"3-1-3",weight:"6-8RM",rest:"2 min",unilateral:false,cue:"Gastrocnemius HSR. 3s down to a deep stretch, 1s pause, 3s up at a true 6-8RM. Drop tempo before you ever drop load."},
            {id:"dA-soleus-hsr",name:"Heavy-Slow Calf Raise — Seated (soleus)",equip:"Machine",sets:3,reps:"6-8",tempo:"3-1-3",weight:"6-8RM",rest:"2 min",unilateral:false,cue:"Bent knee = soleus HSR. Same 3-1-3 tempo, genuinely heavy. The non-redundant second calf angle."},
            {id:"dA-legext-hsr",name:"Leg Extension HSR (patellar)",equip:"Machine",sets:4,reps:"6-8",tempo:"3-1-3",weight:"6-8RM",rest:"2 min",unilateral:false,cue:"Patellar HSR. 3s down, 1s pause, 3s up at a true 6-8RM. Your main patellar stiffness driver on this day."},
            {id:"dA-spanish",name:"Spanish Squat Iso",equip:"Band + plate",sets:4,reps:"1",hold:"40s",weight:"+20kg",rest:"60s",cue:"Band behind the knees, sit back against it, hold at ~90°. Patellar stiffness driver. Add load to progress, keep the hold ~40s."},
          ]},
          {label:"Day B",day:"Sprint / Horizontal + HSR",exercises:[
            {id:"dB-warmup",name:"Warm-up (see Warm-Up tab)",equip:"Warm-up",sets:1,reps:"8-10 min",weight:"",rest:"0s",unilateral:false,cue:"Run the shared Warm-Up first. Day B ramp: 2-3 build-up runs (60→80→95%) + 1-2 near-max efforts before the working accelerations."},
            {id:"dB-accel",name:"Accelerations (20m)",equip:"Sprint",sets:6,metric:"distance",dist:"20m",effort:"~95%",reps:"20m",weight:"",rest:"2 min",cue:"~95% effort over 20m. Walk back between reps to fully recover. Progress sprint speed, never rep count — set ends the moment speed drops."},
            {id:"dB-flying",name:"Flying Sprints (15m run-in + 20m)",equip:"Sprint",sets:4,metric:"distance",dist:"20m",effort:"max",reps:"20m",weight:"",rest:"3 min",cue:"15m run-in to build speed, then 20m at max velocity. Full 3 min rest — this is quality, not conditioning."},
            {id:"dB-altbound",name:"Alternate-Leg Bounding (20m)",equip:"Bodyweight",sets:3,metric:"distance",dist:"20m",effort:"max",reps:"20m",weight:"",rest:"2 min",cue:"Explosive bounding over 20m, alternating legs — horizontal elastic SSC. Big arm drive, cover ground. Walk back between reps."},
            {id:"dB-skater",name:"Lateral / Skater Bounds",equip:"Bodyweight",sets:3,reps:"8 (4/side)",weight:"Bodyweight",rest:"75s",cue:"Explosive push off one leg, land and stick on the other. 4 per side. Multidirectional reactive power — direct kickboxing carryover."},
            {id:"dB-decline-hsr",name:"Single-Leg Decline Squat HSR (patellar)",equip:"Decline board",sets:3,reps:"8-10",tempo:"3-1-3",weight:"6-8RM",rest:"90s",single:true,cue:"Heel raised on a decline, slow 3-1-3 descent, 8-10 per leg. The classic patellar tendon loader — second patellar angle for the week."},
            {id:"dB-calf-hsr",name:"Heavy-Slow Calf Raise — Standing (gastroc)",equip:"Machine/DB",sets:4,reps:"6-8",tempo:"3-1-3",weight:"6-8RM",rest:"2 min",unilateral:false,cue:"Gastrocnemius HSR again — calves tolerate the frequency. 3s down to stretch, 1s pause, 3s up at a true 6-8RM."},
            {id:"dB-wallsit",name:"Wall Sit Iso",equip:"Load",sets:3,reps:"1",hold:"45s",weight:"+20kg",rest:"60s",unilateral:false,cue:"Back flat on the wall, thighs parallel, plate held on the lap. Hold 45s loaded — third patellar / quad-tendon angle for the week."},
            {id:"dB-soleus-iso",name:"Single-Leg Soleus Iso — Bent Knee (optional)",equip:"Single leg + load",sets:3,reps:"1",hold:"30s",weight:"+30kg",rest:"60s",single:true,cue:"Optional. Knee bent ~20-30° to bias the soleus, hold mid-range under load 30s/leg. Loads the deeper Achilles the standing work misses."},
          ]},
        ]},
    };
    ...
    function TendonsTab({weights,saveWeight,counts,setActiveRoutine,theme,reloadLogs,notes,saveNote,tileColor}) {
      ...
      const [tendonData,setTendonData]=useState(()=>{
        ...
        const saved=store.get("tendon_custom_v4",null);
      ...
      const [libQuery,setLibQuery]=useState("");
      const closeLib=()=>{setLibFor(null);setLibPicked({});setLibQuery("");};
      const saveTendon=u=>{setTendonData(u);store.set("tendon_custom_v4",u);};
